#!/usr/bin/env python
"""
https://raw.githubusercontent.com/wiseman/py-webrtcvad/master/example.py
"""
import os
import collections
import contextlib
...
def read_wave(path):
    with contextlib.closing(wave.open(path, 'rb')) as wf:
        ...
        assert sample_rate in (8000, 16000, 32000)#, 44000)
        pcm_data = wf.readframes(wf.getnframes())
        return pcm_data, sample_rate


def write_wave(path, audio, sample_rate, stt=False):
    ...
    if stt:
        ...
        if text:
            with contextlib.closing(open(path+'.txt', 'wb')) as fout:
                fout.write(text)
...
def main(args):
    ...
    for i, segment in enumerate(segments):
        path = '%s-chunk-%002d.wav' % (fn_base, i,)
