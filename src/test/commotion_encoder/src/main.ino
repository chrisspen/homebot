...
byte request=B00101101;        // status request = encoder counts(1) + motor currents(4) + battery voltage(8) + error(32)
...
void ReceiveStatus(int bytes) // empty I²C buffer into datapack
{
	...
	Serial.println("Receiving I2C!"); Serial.flush();
...
/*
    ...
    {
		Serial.println("Received encoders!"); Serial.flush();
    ...
    {
		Serial.println("Received currents!"); Serial.flush();
    ...
    {
		Serial.println("Received analog1!"); Serial.flush();
    ...
    {
		Serial.println("Received analog2!"); Serial.flush();
    ...
    if(request&B00100000)             // request the error log
    {
        ...
        Serial.println(String("Error: ") + String(errorlog)); Serial.flush();
    }
    */
}
...
void ComMotionStatus2(byte mcu){
	
	Serial.println("status2.start"); Serial.flush();
	Wire.requestFrom(COMMOTION_ADDR + COMMOTION_OFFS + mcu, 8);    // request 6 bytes from slave device #8

	Wire.read();
	int acount = get_wire_int();
	Wire.read();

	Wire.read();
	int bcount = get_wire_int();
	Wire.read();
	  
	Serial.println(String("acount:")+String(acount));
	Serial.println(String("bcount:")+String(bcount));
	Serial.println("status2.done"); Serial.flush();
}
...
void loop()
{
	...
	Serial.println(String("received_bytes:") + String(received_bytes)); Serial.flush();
	
	Serial.println("Stopping..."); Serial.flush();
	...
	Serial.println("Backward..."); Serial.flush();
	IndividualMotorControl(0, 0, -speed, -speed);
	delay(timeout);

	Serial.println("Turn CW..."); Serial.flush();
	IndividualMotorControl(0, 0, -speed, speed);
	delay(timeout);

	Serial.println("Turn CCW..."); Serial.flush();
	IndividualMotorControl(0, 0, speed, -speed);
	delay(timeout);
	*/
