...
class BNO055:
	...
	def getTemp(self):
		#TODO:filter? usually at 27-28 but sometimes spikes to 155?
...
def format_vector(v, digits=9, decimals=2):
	parts = []
	for part in v:
		parts.append(('%+0'+str(digits)+'.'+str(decimals)+'f') % part)
	return '(' + (', '.join(parts)) + ')'

#TODO:handle i2c errors due to RPi bug like
# Traceback (most recent call last):
#   File "BNO055.py", line 304, in <module>
#     print 'gyr:', format_vector(bno.getVector(BNO055.VECTOR_GYROSCOPE)),
#   File "BNO055.py", line 264, in getVector
#     buf = self.readBytes(vectorType, 6)
#   File "BNO055.py", line 279, in readBytes
#     return self._bus.read_i2c_block_data(self._address, register, numBytes)
# KeyboardInterrupt
...
if __name__ == '__main__':
	...
	print 'calibration:', bno.getCalibration()
	time.sleep(3)
	while True:
		print '%.02f' % time.time(),
		print 'temp: %03i' % bno.getTemp(),
		print 'euler:', format_vector(bno.getVector(BNO055.VECTOR_EULER)),
		print 'acl:', format_vector(bno.getVector(BNO055.VECTOR_ACCELEROMETER)),
		print 'gyr:', format_vector(bno.getVector(BNO055.VECTOR_GYROSCOPE)),
		print 'grv:', format_vector(bno.getVector(BNO055.VECTOR_GRAVITY)),
		print 'mag:', format_vector(bno.getVector(BNO055.VECTOR_MAGNETOMETER))
		time.sleep(0.1)
