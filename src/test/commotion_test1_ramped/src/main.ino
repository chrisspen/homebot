...
void loop()
{
    ...
    MotorController motor_left = MotorController();
    MotorController motor_right = MotorController();

    Serial.println("Forward...");
    motor_left.set_speed(speed);
    motor_right.set_speed(speed);
    future = millis() + timeout;
    while(millis() < future){
        IndividualMotorControl(0, 0, motor_left.get_speed(), motor_right.get_speed());//NA NA left right
        delay(10);
    }
