...
static struct gyro_state_s st =
{
	...
	const struct gyro_reg_s reg =
	{
		...
		prgm_start_h   : 0x70
			...
			i2c_delay_ctrl : 0x67,
			raw_compass    : 0x49,
			yg_offs_tc     : 0x01 //TODO:correct? was missing originally?
		#endif
	};
	const struct hw_s hw =
	{
...
uint8_t mpu_init(struct int_param_s *int_param)
{
	...
	cout << "Using address 0x" << HEX(st.hw->addr) << endl;
	...
	data[0] = 0x00;
	if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data)){
		cout << "Failed to wake up chip." << endl;
		return 1;
	}
	...
	if (i2c_read(st.hw->addr, st.reg->accel_offs, 6, data)){
		cout << "Invalid product revision." << endl;
		return 1;
	}
	...
	if (mpu_read_mem(MPU6500_MEM_REV_ADDR, 1, &rev)){
		cout << "Invalid mem rev addr." << endl;
		rev = 1;//TODO:fix? fails on the MPU9250?
//		return 1;
	}
	if (rev == 0x1)
		st.chip_cfg.accel_half = 0;
	...
	if (i2c_write(st.hw->addr, st.reg->accel_cfg2, 1, data)){
		cout << "Unable to write to register accel_cfg2." << endl;
		return 1;
	}
#endif
	...
	if (mpu_set_gyro_fsr(2000)){
		cout << "Unable to set gyro fsr=2000." << endl;
		return 1;
	}
	if (mpu_set_accel_fsr(2)){
		cout << "Unable to set accel fsr=2." << endl;
		return 1;
	}
	if (mpu_set_lpf(42)){
		cout << "Unable to set lpf=42." << endl;
		return 1;
	}
	if (mpu_set_sample_rate(50)){
		cout << "Unable to set sample rate." << endl;
		return 1;
	}
	if (mpu_configure_fifo(0)){
		cout << "Unable to set fifo." << endl;
		return 1;
	}
	...
	if (mpu_set_compass_sample_rate(10)){
		cout << "Unable to set compass sample rate=10." << endl;
		return 1;
	}
#else
	/* Already disabled by setup_compass. */
	if (mpu_set_bypass(0)){
		cout << "Unable to set bypass." << endl;
		return 1;
	}
#endif
...
uint8_t mpu_read_fifo_stream(uint16_t length, uint8_t *data, uint8_t *more)
{
	...
	if (!st.chip_cfg.dmp_on)
    {
		cout << "!st.chip_cfg.dmp_on" << endl;
		return 1;
    }
	if (!st.chip_cfg.sensors)
    {
		cout << "!st.chip_cfg.sensors" << endl;
		return 1;
    }

	if (i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, tmp)){
		cout << "st.hw->addr, st.reg->fifo_count_h" << endl;
		return 1;
	}
	...
	if (fifo_count > (st.hw->max_fifo >> 1))
	{
		...
		if (i2c_read(st.hw->addr, st.reg->int_status, 1, tmp)){
			cout << "st.reg->int_status" << endl;
			return 1;
		}
		if (tmp[0] & BIT_FIFO_OVERFLOW)
		{
			mpu_reset_fifo();
			cout << "BIT_FIFO_OVERFLOW" << endl;
			return 2;
		}
	}
#if defined MPU_DEBUG
    printf_P("FIFO count 2: %hd\r\n", fifo_count);
#endif
	...
	cout << "fifo more: " << fifo_count / length - 1 << endl;
...
uint8_t mpu_run_self_test(int32_t *gyro, int32_t *accel)
{
	...
	uint16_t gyro_fsr, sample_rate = 10, lpf;
	uint8_t dmp_was_on;
