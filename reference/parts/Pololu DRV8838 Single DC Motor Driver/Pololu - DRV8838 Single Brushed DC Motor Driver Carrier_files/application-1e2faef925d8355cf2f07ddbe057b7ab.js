...
function updatePictureScroll()
{
    ...
    if(pictures && pictures.offsetWidth < p.offsetWidth)
    {
    ...
    if(p && p.style.marginLeft == "0px")
    {
	if(left) left.style.display = "none";
    }
    if(p && parseInt(p.style.marginLeft) + p.offsetWidth <= pictures.offsetWidth + 3)
    {
	if(right) right.style.display = "none";
    }
}
...
function pictureView(imgurl, link, longest_side)
{
    console.log('pictureZoom('+link+','+longest_side+')');
    ...
    main_picture_a.setAttribute('onclick', 'pictureZoom(\''+link+'\','+longest_side+');return false;');

    return false;
}

function pictureZoom(url, longest_side) 
{
  var window_size = (longest_side<=800) ? 840 : 1240;

  var width = window_size;
  var height = window_size + 135;

  width_max = Math.round(0.9*screen.width);
  height_max = Math.round(screen.height-110);
  ...
  var options = 'height=' + height + ',width=' + width +',scrollbars=1,resizable=1';
...
function loadMath() {
  ...
  if (loadMathjax && (typeof MathJax === 'undefined')) {
    $("#mathjax").append( '<script type="text/x-mathjax-config">\n'+
                          ...
                          '</script>\n' +
                          '<script src="' + mathjax_src + '" type="text/javascript"></script>')
  }
}
